...
@assertion(
    ...
    preconditions="Admin bound; target is writable; assertion control advertised.",
    stimulus="Add a test entry, then ModifyRequest with an assertion control whose filter is TRUE (present objectClass).",
    expected_observables="ModifyResponse success; entry removed in cleanup.",
    mutates=True,
    layer=Layer.WIRE,
    oid="1.3.6.1.1.12",
)
...
@assertion(
    ...
    preconditions="Admin bound; target is writable; assertion control advertised.",
    stimulus="Add a test entry, then DelRequest with an assertion control whose filter is TRUE.",
    expected_observables="DelResponse success; entry removed in cleanup.",
    mutates=True,
    layer=Layer.WIRE,
    oid="1.3.6.1.1.12",
)
...
@assertion(
    ...
    preconditions="Assertion control advertised; seed entry uid=alice exists.",
    stimulus="SearchRequest with an assertion control whose filter is FALSE (NOT present objectClass).",
    expected_observables="SearchResultDone resultCode assertionFailed (122).",
    layer=Layer.WIRE,
    oid="1.3.6.1.1.12",
)
